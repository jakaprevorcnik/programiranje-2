...
//     let mut a = [13; 100]; // 13 se pojavi prvic
//     let mut b = a;         // b prevzame lastnistvo nad a
//     let q = String::from("13"); // naredimo string 13 
//     println!("{}", q) ; 
//     let r = q; // 
//     let p = &r;
//     a[0] = 1;
...
fn swap (a: &mut i32, b: &mut i32) { //ce spreminjamo a b morata bit muable zato si moramo sposodit mut nekje ker ga v arg nemoremo navesti kot tip
    let temp = *a; //def zacasno spremenljivko ki kaze na vrednost od a
    *a = *b; //
    *b = temp
}
// fn test_swap() {
...
// /// Popravite zakomentiran del spodnje funkcije, da bo deloval
  fn str_own() {
       let x = String::from("Hello world"); // -samo "Helo world" ni spremenljiva za razliko od String.from("Hello world")
...
// // /// Popravite spodnjo funkcijo, da bo delovala
 ...
 //fn fn2() {
     ...
     // assert_eq!(*x, 5); //assert_eq ce bo enako ne bo naredil nic drugace sicer se sesuje
...
//     println!("{:p}",&x) //ukaz da vidimo kje v pomnilniku se nahaja x !rabimo referenco zakaj??
//  }
...
//       assert_eq!(13, *y); // za vsako & rabimo eno * ce hocemo dostopat do vrednosti same
...
//       let r2 = &mut s; //moramo si sposodit al nickrat ali pa samo enkrat
...
//       let mut s = String::from("hello, "); //rabmo s mut ker si ga spodaj sposodimo kot mut
